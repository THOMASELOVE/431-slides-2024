...
```{r}
#| echo: true
fit1 <- lm((100/a1c) ~ a1c_old, data = dm500_i_train)

fit2 <- lm((100/a1c) ~ a1c_old + age, data = dm500_i_train)

fit3 <- lm((100/a1c) ~ a1c_old + age + income, 
            data = dm500_i_train)
...
aug1 <- augment(fit1, data = dm500_i_train) |>
  mutate(inv_a1c = 100/a1c) # add in our model's outcome
```

`aug1` includes all variables in `dm500_i_train` and also:

- `inv_a1c` = 100/`a1c`, transformed outcome `fit1` predicts
...
## `augment` results: last 6 subjects
...
aug1 |> tail() |> select(subject, .std.resid, inv_a1c)
```
...
aug1 |> select(inv_a1c, .fitted:.std.resid) |> summary()
```
...
aug1 |> slice(214)
aug1 |> slice(214) |> select(subject, .std.resid, inv_a1c)
```
...
aug1 |> slice(198)
aug1 |> slice(198) |> select(subject, .std.resid, inv_a1c)
```
...
aug2 |> select(inv_a1c, .fitted:.std.resid) |> summary()
```
...
aug2 |> slice(214)
aug2 |> slice(214) |> select(subject, .std.resid, inv_a1c)
```
...
aug2 |> slice(198)
aug2 |> slice(198) |> select(subject, .std.resid, inv_a1c)
```
...
aug3 |> select(inv_a1c, .fitted:.std.resid) |> summary()
```
...
aug3 |> slice(214)
aug3|> slice(214) |> select(subject, .std.resid, inv_a1c)
```
...
aug3 |> slice(198)
aug3 |> slice(198) |> select(subject, .std.resid, inv_a1c)
```
...
slice(aug1, c(1, 198))
slice(aug1, c(1, 198)) |> select(subject, .std.resid, inv_a1c)
```
...
slice(aug2, c(1, 198))
slice(aug2, c(1, 198)) |> select(subject, .std.resid, inv_a1c)
```
...
slice(aug3, c(1, 198))
slice(aug3, c(1, 198)) |> select(subject, .std.resid, inv_a1c)
```
...
#| echo: true
#| message: true
temp <- dm500_i_train |> mutate(inv_a1c = 100/a1c) |> 
...
ggplot(aug1, aes(x = .fitted, y = inv_a1c)) +
  geom_point() +
...
```{r}
#| echo: true
aug1 |> reframe(lovedist(inv_a1c))

aug1 |> reframe(lovedist(.fitted))

cor(aug1$inv_a1c, aug1$.fitted)
```
...
p2 <- ggplot(aug1, aes(x = inv_a1c, y = "")) +
  geom_violin() + 
...
p2 <- ggplot(aug2, aes(x = inv_a1c, y = "")) +
  geom_violin() + 
  geom_boxplot(fill = "mistyrose", alpha = 0.5, width = 0.3) + 
  xlim(0, 25) +
  stat_summary(fun = "mean", col = "red") +
